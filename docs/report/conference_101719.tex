...
\textbf{Normalization matters more than optimizer choice.} The second most impactful improvement was changing from per-sample to global z-normalization (+0.88\%), which outperformed optimizer changes (+0.23\% for AdamW). This finding is significant because normalization strategy is rarely the focus of ablation studies, yet it had a larger effect than standard hyperparameter tuning. Global statistics provide more stable scaling, particularly for short time series where per-sample statistics may be unreliable.
